...
export default function BrandLogo({ className, withText = true }: BrandLogoProps) {
  return (
    <div className={cx('inline-flex items-center gap-3', className)}>
      <span className="grid h-9 w-9 place-items-center rounded-md border border-[#333] bg-[#111]">
        <svg viewBox="0 0 48 48" className="h-5 w-5" aria-hidden="true">
          ...
          <path
            ...
            fill="#333"
          />
          <circle cx="24" cy="24" r="5.2" fill="#111" />
          <circle cx="24" cy="24" r="2.6" fill="#3b82f6" />
        </svg>
      </span>
      {withText ? (
        <span>
          <span className="block text-[11px] uppercase tracking-wider text-[#3b82f6]">
            tracker
          </span>
